...
test.describe("글로키 리스트 페이지", () => {
  ...
  test("글로키 리스트 페이지를 보여준다.", async ({ page }) => {
    await expect(page.getByText("총 글로키 수 6")).toBeVisible();
    await expect(page.getByText("총 이용가능 글로키 수 6")).toBeVisible();
  });
  ...
  test("글로키 업로드하기를 클릭하면 다음 글로키를 지정할 수 있는 폼을 보여준다", async ({
    ...
    await expect(page.getByText("글로키 업로드하기")).toBeVisible();
  });
});
